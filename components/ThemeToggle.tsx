...
export default function ThemeToggle() {
  ...
  return (
    <label
      className={`swap swap-rotate p-1 ${mounted ? "opacity-100" : "opacity-0"}`}
      aria-hidden={!mounted}
    >
      <input
        ref={inputRef}
      ...
      />
      <svg
        className={`swap-off h-6 w-6 fill-current ${noTransitionClass}`}
        viewBox="0 0 24 24"
      >
      ...
      <svg
        className={`swap-on h-6 w-6 fill-current ${noTransitionClass}`}
        viewBox="0 0 24 24"
      >
