...
export default function EventsList({
  events,
  weekBoundaries: { start, end, nextEnd },
}: {
  ...
  const thisWeek = events.filter(
    (e) =>
      !!e.startsAt &&
      isAfter(e.startsAt, thisWeekStart) &&
      isBefore(e.startsAt, thisWeekEnd),
  );
  const nextWeek = events.filter(
    (event) =>
      !!event.startsAt &&
      isAfter(event.startsAt, thisWeekEnd) &&
      isBefore(event.startsAt, nextWeekEnd),
  );
  const later = events.filter(
    (event) => !!event.startsAt && isAfter(event.startsAt, nextWeekEnd),
  );
